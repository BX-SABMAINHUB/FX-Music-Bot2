...
VIP_ROLE_NAME = "VIP" # El nombre del rol que no escuchará anuncios
...
class FlexusBot(commands.Bot): 
    ...
    async def setup_hook(self): 
        ...
        print(f"💎 FLEXUS V9.0: HI-FI 192KBPS & VIP SYSTEM READY") 
...
# MEJORA: 192kbps y buffer optimizado para evitar cortes
FFMPEG_OPTIONS = {
...
class ReviewModal(ui.Modal, title="⭐ VALORACIÓN PREMIUM ⭐"):
    ...
    stars = ui.TextInput(label="Puntuación (1-5)", placeholder="⭐⭐⭐⭐⭐", min_length=1, max_length=1)
    reason = ui.TextInput(label="Feedback del Audio", style=discord.TextStyle.paragraph, placeholder="¿Cómo se escuchó el Hi-Fi?", min_length=5)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title="✅ RESEÑA PUBLICADA", description="Tu valoración ha sido enviada al Dashboard de Vercel.", color=0x00ff77)
        await interaction.response.send_message(embed=embed, ephemeral=True)

# --- LÓGICA DE REPRODUCCIÓN (FIX CORTES + ANUNCIO VIP) ---
def play_next(interaction, channel_id, last_user):
    if not interaction.guild.voice_client: return
    
    # Lanzar reseña al terminar
    if bot.current_track and not bot.loop_mode:
        track = bot.current_track
        async def send_review():
            chan = bot.get_channel(channel_id)
            if chan:
                view = ui.View().add_item(ui.Button(label="Valorar Sonido ⭐", style=discord.ButtonStyle.success, emoji="🎧"))
                async def cb(i): await i.response.send_modal(ReviewModal(track))
                view.children[0].callback = cb
                emb = discord.Embed(title="📀 PISTA FINALIZADA", description=f"Acabas de escuchar: **{track}**\n¿Qué te pareció la calidad 192k?", color=0x00ff77)
                await chan.send(embed=emb, view=view)
        bot.loop.create_task(send_review())

    # LÓGICA DE ANUNCIO (SALTO PARA VIPs)
    ...
    if not is_vip and bot.songs_played >= 3 and os.path.exists("anuncio.mp3"):
        bot.songs_played = 0
        emb_ads = discord.Embed(title="📢 ANUNCIO FLEXUS", description="Reproduciendo publicidad... \n*Consigue el rol VIP para eliminar anuncios.*", color=0xffff00)
        bot.loop.create_task(interaction.channel.send(embed=emb_ads))
    ...
    if bot.queue:
        url, title, user_who_added = bot.queue.pop(0)
        bot.current_track = title
        bot.songs_played += 1
        data = ytdl.extract_info(url, download=False)
        source = discord.FFmpegPCMAudio(data['url'], **FFMPEG_OPTIONS)
        interaction.guild.voice_client.play(source, after=lambda e: play_next(interaction, channel_id, user_who_added))
    else:
        bot.current_track = None

# --- LOS 19 COMANDOS CON EMBEDS ---
@bot.tree.command(name="play", description="🎶 Sonido Hi-Fi 192k + VIP System")
async def play(interaction: discord.Interaction, buscar: str):
    ...
    class MusicMenu(ui.Select):
        def __init__(self):
            super().__init__(placeholder="💎 Selecciona la calidad Premium...", options=[
                discord.SelectOption(label=r['title'][:90], emoji="💿", value=str(i)) for i, r in enumerate(results)
            ])
        async def callback(self, inter: discord.Interaction):
            if inter.user.id != u_id: 
                return await inter.response.send_message("❌ No es tu búsqueda.", ephemeral=True)
            ...
            if vc.is_playing():
                ...
                emb = discord.Embed(title="🚀 REPRODUCCIÓN INICIADA", description=f"**{s['title']}**\nCalidad: `192kbps Hi-Fi` 💎", color=0x00ff77)
                await inter.followup.send(embed=emb)

    await interaction.followup.send(view=ui.View().add_item(MusicMenu()))

@bot.tree.command(name="skip")
async def skip(i: discord.Interaction):
    if i.guild.voice_client: i.guild.voice_client.stop()
    await i.response.send_message(embed=discord.Embed(description="⏭️ **Pista saltada con éxito.**", color=0x00ff77))

@bot.tree.command(name="stop")
async def stop(i: discord.Interaction):
    bot.queue.clear()
    if i.guild.voice_client: await i.guild.voice_client.disconnect()
    await i.response.send_message(embed=discord.Embed(description="⏹️ **Reproductor detenido.**", color=0xff0000))
...
async def q(i: discord.Interaction):
    txt = "\n".join([f"**{idx+1}.** {t[1]}" for idx, t in enumerate(bot.queue[:10])]) or "No hay canciones."
...
async def vol(i: discord.Interaction, v: int):
    ...
    await i.response.send_message(embed=discord.Embed(description=f"🔊 **Volumen ajustado al {v}%**", color=0x00ff77))
...
async def cl(i: discord.Interaction):
    bot.queue.clear()
    await i.response.send_message(embed=discord.Embed(description="🗑️ **Cola vaciada.**", color=0xff0000))

@bot.tree.command(name="leave")
async def lv(i: discord.Interaction):
    if i.guild.voice_client: await i.guild.voice_client.disconnect()
    await i.response.send_message(embed=discord.Embed(description="👋 **Flexus se ha desconectado.**", color=0xff4444))
...
async def bb(i: discord.Interaction):
    await i.response.send_message(embed=discord.Embed(title="🔥 BASS BOOST", description="Filtros de bajos profundos aplicados.", color=0xe67e22))

@bot.tree.command(name="loop")
async def lp(i: discord.Interaction):
    bot.loop_mode = not bot.loop_mode
    await i.response.send_message(embed=discord.Embed(description=f"🔁 **Bucle:** `{'ON' if bot.loop_mode else 'OFF'}`", color=0x00ff77))

@bot.tree.command(name="lyrics")
async def lyr(i: discord.Interaction):
    await i.response.send_message(embed=discord.Embed(title="🔍 LETRAS", description=f"Buscando letras para **{bot.current_track}**...", color=0x3498db))

@bot.tree.command(name="stats")
async def st(i: discord.Interaction):
    await i.response.send_message(embed=discord.Embed(title="📊 ESTADÍSTICAS", description=f"Canciones hoy: `{bot.songs_played}`", color=0x00ff77))

@bot.tree.command(name="help")
async def help(i: discord.Interaction):
    emb = discord.Embed(title="👑 COMANDOS PREMIUM", color=0x00ff77)
    emb.add_field(name="Audio", value="`play`, `skip`, `stop`, `pause`, `resume`, `volume`, `loop`, `restart`, `bassboost`", inline=False)
    emb.add_field(name="Cola", value="`queue`, `shuffle`, `clear`, `jump`", inline=False)
    emb.add_field(name="Info", value="`nowplaying`, `lyrics`, `stats`, `ping`, `help`, `leave`")
